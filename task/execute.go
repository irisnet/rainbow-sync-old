...
func (s *TaskZoneService) StartExecuteTask() {
	var (
...
func (s *TaskZoneService) executeTask(blockNumPerWorkerHandle, maxWorkerSleepTime int64, chanLimit chan bool) {
	var (
	...
	tasks, err := s.syncModel.GetExecutableTask(maxWorkerSleepTime)
	if err != nil {
	...
	task := tasks[0]
	err = s.syncModel.TakeOverTask(task, workerId)
	if err != nil {
		...
		func() {
			for {
				...
				case <-healthCheckQuit:
					...
					return
				default:
					task, err := s.syncModel.GetTaskByIdAndWorker(taskId, workerId)
					if err == nil {
						if _, valid := s.assertTaskValid(task, blockNumPerWorkerHandle); valid {
							// update task last update time
							if err := s.syncModel.UpdateLastUpdateTime(task); err != nil {
	...
	blockChainLatestHeight, isValid := s.assertTaskValid(task, blockNumPerWorkerHandle)
	for isValid {
...
func (s *TaskZoneService) assertTaskValid(task cmodel.SyncZoneTask, blockNumPerWorkerHandle int64) (int64, bool) {
	var (
		taskType               string
		flag                   = false
		blockChainLatestHeight int64
...
func assertTaskWorkerUnchanged(taskId bson.ObjectId, workerId string) (bool, error) {
	var (
		syncTaskModel cmodel.SyncZoneTask
	)
	// check task owner
